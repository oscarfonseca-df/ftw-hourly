...
describe('richText', () => {
  ...
  describe('linkifyOrWrapLinkSplit(word, key, linkClass)', () => {
    ...
    it('should add link around words that are links', () => {
      ...
      expect(wrapper.html()).toEqual(
        '<span><a class="linkClass" href="http://www.example.com" rel="noopener noreferrer" target="_blank">http://www.example.com</a></span>'
      );
    });
  ...
  describe('richText(text, { longWordMinLength, longWordClass })', () => {
    ...
    it('should add span around a string with a long word, containing slashes and a link', () => {
      ...
      // </span>
      expect(wrapper.html()).toEqual(
        `<span>Chars one${slashWithZWSP}two${slashWithZWSP}three - count until <span class=\"longWord\">exhaustion…</span> and a random link: <a class=\"link\" href=\"http://www.example.com\" rel=\"noopener noreferrer\" target=\"_blank\">http://www.example.com</a></span>`
      );
    });
    it('should add link inside non-whitespace-sequence (http://example.com)', () => {
      ...
      // </span>
      expect(wrapper.html()).toEqual(
        `<span>Link: (<a class=\"longWord\" href=\"http://example.com\" rel=\"noopener noreferrer\" target=\"_blank\">http://example.com</a>)</span>`
      );
    });
