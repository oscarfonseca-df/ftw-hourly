...
const checkSelected = (options, selectedOptions) => options.map(option => ({
    key: option.key,
    label: option.label,
...
const IconCheck = props => {
  ...
  return (
    <svg className={classes} height="9" width="9" xmlns="http://www.w3.org/2000/svg">
      <path
...
const PropertyGroup = props => {
  ...
  return (
    <ul className={listClasses}>
      {checked.map(option => (
        <Item key={`${id}.${option.key}`} isSelected={option.isSelected} label={option.label} />
      ))}
    </ul>
  );
};
